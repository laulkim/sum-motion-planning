...
class PlannerNodeCpp final : public rclcpp::Node {
 ...
 private:
  ...
  void odom_callback(const nav_msgs::msg::Odometry::SharedPtr msg) {
    ...
    {
      std::lock_guard<std::mutex> lock(input_mutex_);
      tracking_odom_state_ = {msg->pose.pose.position.x, msg->pose.pose.position.y,
                              chi, speed, acceleration, heading_rate};
      tracking_odom_body_yaw_ = body_yaw;
      tracking_odom_yaw_rate_ = msg->twist.twist.angular.z;
      tracking_odom_time_ns_ = rclcpp::Time(msg->header.stamp).nanoseconds();
      if (tracking_odom_time_ns_ <= 0) tracking_odom_time_ns_ = now_ns();
      // Feedback uses live odometry; planning is seeded only once and then
      // advances through predicted handover states, including spot turns.
      if (received_odom_) return;
      current_state_ = tracking_odom_state_;
      current_body_yaw_ = body_yaw;
      current_body_yaw_rate_ = tracking_odom_yaw_rate_;
      current_state_time_ns_ = tracking_odom_time_ns_;
  ...
  void command_callback() {
    ...
    ReferencePathMsg::SharedPtr deferred;
    {
      ...
      tracking_state = tracking_odom_state_;
      tracking_body_yaw = tracking_odom_body_yaw_;
      tracking_body_yaw_rate = tracking_odom_yaw_rate_;
      odom_stamp_ns = tracking_odom_time_ns_;
    ...
    const bool tracking_state_fresh = odom_stamp_ns > 0 && odom_age >= 0.0;
      ...
      if (forced_safety_stop_) {
        ...
        if (elapsed <= active->allocation.trajectory.t.back() + 1.0e-12) {
          command = sample_body_command(active->allocation,
                                        ...
                                        elapsed, command_dt_);
          if (tracking_state_fresh && command->planned_speed > 0.0) {
            ...
            command = apply_tracking_feedback(
                *command, tracking_state, tracking_config_);
          }
        } else {
          if (!safety_stop_) {
  ...
  EnvConfig config_;
  PlannerState tracking_odom_state_;
  double tracking_odom_body_yaw_{0.0};
  double tracking_odom_yaw_rate_{0.0};
  std::int64_t tracking_odom_time_ns_{0};
